@push('scripts')
<script src="{{ asset('js/document-viewer.js') }}"></script>
@endpush
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 flex items-center justify-between">
            ...
            <div class="flex space-x-3">
                <a href="{{ route('dean.pending-reviews') }}" class="inline-flex items-center px-4 py-2 bg-gray-600 border border-transparent rounded-lg font-semibold text-sm text-white uppercase tracking-widest hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-all duration-200">
                    <i class="fas fa-arrow-left mr-2"></i> Back to Reviews
                </a>
                @if($activity->workflow_status === 'noted_by_adviser')
                    <button onclick="openReviewModal({{ $activity->id }}, '{{ addslashes($activity->title) }}')" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-lg font-semibold text-sm text-white uppercase tracking-widest hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-all duration-200">
                        <i class="fas fa-eye mr-2"></i> Note & Forward
                    </button>
                    <button onclick="openRejectModal({{ $activity->id }}, '{{ addslashes($activity->title) }}')" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-lg font-semibold text-sm text-white uppercase tracking-widest hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-all duration-200">
            ...
            <div class="p-6">
                <div class="flex items-center justify-between">
                    ...
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium
                        ...
                        @elseif($activity->workflow_status === 'noted_by_adviser') bg-purple-100 text-purple-800
                ...
                <div class="space-y-2">
                    @if($activity->objective_1)
                        <div class="flex items-start">
                            <span class="flex-shrink-0 w-6 h-6 bg-purple-100 text-purple-800 rounded-full flex items-center justify-center text-xs font-medium mr-3 mt-0.5">1</span>
                            <p class="text-sm text-gray-900">{{ $activity->objective_1 }}</p>
                        </div>
                    @endif
                    @if($activity->objective_2)
                        <div class="flex items-start">
                            <span class="flex-shrink-0 w-6 h-6 bg-purple-100 text-purple-800 rounded-full flex items-center justify-center text-xs font-medium mr-3 mt-0.5">2</span>
                            <p class="text-sm text-gray-900">{{ $activity->objective_2 }}</p>
                        </div>
                    @endif
                    @if($activity->objective_3)
                        <div class="flex items-start">
                            <span class="flex-shrink-0 w-6 h-6 bg-purple-100 text-purple-800 rounded-full flex items-center justify-center text-xs font-medium mr-3 mt-0.5">3</span>
                            ...
                            <div class="relative">
                                <div class="relative flex space-x-3">
                                    ...
                                    <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                        ...
                                        <div class="text-right text-sm whitespace-nowrap text-gray-500">
                                            {{ $activity->dean_noted_at ? $activity->dean_noted_at->format('M d, Y g:i A') : 'Pending' }}
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="mt-2 px-7 py-3">
                ...
                <form id="reviewForm" method="POST" class="mt-4">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="notes" id="reviewNotes" rows="3" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500" placeholder="Add any notes for this review..."></textarea>
                    </div>
